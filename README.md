...
**5. The visualization is engine-composed too — the engine computes the scene, the browser renders it.** The 3D globe follows the same discipline as every number: the engine composes a CZML document (`GET /api/events/{id}/czml` — every coordinate computed, every orbit propagated), and the browser treats it as opaque data and only draws it. The React layer never re-derives orbit geometry, and there is deliberately **no sample CZML fallback** — a fabricated orbit would be a fabricated number. The same engine rows drive the globe, the B-plane figure, and the event board, so every picture of an event agrees with every number beside it.
...
| `GET` | `/api/events/{id}/czml?maneuver_kind=&window_min=` | Engine-composed CZML scene for the 3D globe — orbits over ±window, TCA marks, miss line, velocity arrow, covariance ellipsoid, optional maneuver pre/post-burn track |
